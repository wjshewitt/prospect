...
import {
  analyzeTerrainForBuilding,
  estimateExcavationVolume,
...
interface TerrainAnalysisPanelProps {
  ...
  onBuildingPadsFound?: (
    pads: { coordinates: number[]; sizeSquareMeters: number }[]
  ) => void;
}
        ...
        <div className="space-y-2 text-sm">
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-lg bg-muted p-2">
              <div className="text-xs text-foreground">Max Slope</div>
              <div>{analysisResults.maxSlope.toFixed(1)}°</div>
            </div>
            <div className="rounded-lg bg-muted p-2">
              <div className="text-xs text-foreground">Mean Slope</div>
          ...
          {analysisResults.cutVolume !== undefined && (
            <div className="rounded-lg bg-muted p-2">
              <div className="text-xs text-foreground">Estimated Earthwork</div>
              <div>
                Cut: {(analysisResults.cutVolume / 1000).toFixed(1)}k m³
              </div>
              <div>
                Fill: {(analysisResults.fillVolume! / 1000).toFixed(1)}k m³
              </div>
            </div>
          )}
        </div>
