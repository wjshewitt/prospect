...
export function ElevationAnalysis({
  ...
  const analysis = useMemo(() => {
    if (!elevationGrid || !elevationGrid.cells) {
      return { flatPercent: 0, steepPercent: 0, totalCells: 0, minSlope: 0, maxSlope: 0 } as any;
    }
    ...
    if (validCells.length === 0) {
      return {
        ...
        minSlope: 0,
        maxSlope: 0,
        invalidCells: elevationGrid.cells.length,
      } as any;
    }
  ...
  if (selectedShapeIds.length !== 1) {
    return (
      <Card>
        ...
        <CardContent>
          <CardDescription>Select a single shape to analyze its topography.</CardDescription>
        </CardContent>
      </Card>
    );
  }
          ...
          <div className="flex items-center justify-between space-x-2">
            <CollapsibleTrigger asChild>
              <button className="flex items-center gap-2 flex-1">
                ...
                <Label htmlFor="analysis-visibility" className="cursor-pointer">
                  Show Grid
                </Label>
              </button>
            </CollapsibleTrigger>
            <Switch id="analysis-visibility" checked={isAnalysisVisible} onCheckedChange={setIsAnalysisVisible} />
          </div>
            ...
            <div className="flex justify-around text-center">
              <div>
                <p className="text-lg font-bold flex items-center justify-center gap-1">
                  ...
                  {elevationGrid.minElevation?.toFixed(1)}m
                </p>
              ...
              <div>
                <p className="text-lg font-bold flex items-center justify-center gap-1">
                  ...
                  {elevationGrid.maxElevation?.toFixed(1)}m
                </p>
              ...
              <div>
                <p className="text-lg font-bold flex items-center justify-center gap-1">
                  ...
                  {(Number(elevationGrid.maxElevation?.toFixed(1)) - Number(elevationGrid.minElevation?.toFixed(1))).toFixed(1)}m
                </p>
