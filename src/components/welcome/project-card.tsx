...
function computeBBox(shapes: Shape[]) {
  ...
  if (!isFinite(minLat) || !isFinite(minLng) || !isFinite(maxLat) || !isFinite(maxLng)) {
    ...
    minLat = -0.001; minLng = -0.001; maxLat = 0.001; maxLng = 0.001;
  }
...
function colorForZone(kind?: string) {
  switch(kind) {
    ...
    case 'amenity': return '#F59E0B';
    case 'green_space': return '#22C55E';
...
function generatePreviewSVG(shapes: Shape[], title: string) {
  const width = 600, height = 340; // 16:9ish
  const pad = 8;
  ...
  const projectHue = Math.abs(title.split('').reduce((a,c)=>a + c.charCodeAt(0),0)) % 360;
  ...
  shapes.forEach(s => {
    ...
    if (s.zoneMeta) {
      polys.push(`<polygon points="${pts}" fill="${colorForZone(s.zoneMeta.kind)}" fill-opacity="0.35" stroke="${colorForZone(s.zoneMeta.kind)}" stroke-opacity="0.9" stroke-width="1.5" />`);
    } else if (s.assetMeta) {
      polys.push(`<polygon points="${pts}" fill="#A9927D" stroke="#5E503F" stroke-width="1" />`);
    } else {
      polys.push(`<polygon points="${pts}" fill="none" stroke="#FF6B35" stroke-width="2.5" />`);
    }
  });
...
<svg xmlns='http://www.w3.org/2000/svg' width='${width}' height='${height}' viewBox='0 0 ${width} ${height}'>
  ...
  <rect x='${pad}' y='${pad}' width='${width-2*pad}' height='${height-2*pad}' fill='url(#grid)' />
  ${polys.join('\n  ')}
</svg>`;
...
export function ProjectCard({ project }: ProjectCardProps) {
    ...
    const previewUrl = useMemo(() => generatePreviewSVG(project.shapes, project.siteName || 'Project'), [project.shapes, project.siteName]);

    return (
        <Card className="flex flex-col overflow-hidden transition-all hover:shadow-lg hover:-translate-y-1 bg-card">
            <div className="h-1 w-full" style={{background: `linear-gradient(90deg, hsl(160 60% 45%), hsl(30 95% 55%))`}} />
            ...
            <CardContent className="flex-grow space-y-3">
                <div className="aspect-video w-full overflow-hidden rounded-md border bg-muted">
                    ...
                    <img src={previewUrl} alt={`Preview of ${project.siteName}`} className="h-full w-full object-cover" />
                </div>
